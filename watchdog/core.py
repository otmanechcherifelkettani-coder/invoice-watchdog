...
def audit(invoices,limit=5):
    ...
    positive_impact=money(sum(max(0,s['impact']) for s in signals))
    return {'signals':signals[:limit],'all_signal_count':len(signals),'possible_monthly_impact':positive_impact,'reviews':reviews,'invoices':[{'supplier':i['supplier'],'invoice_id':i['invoice_id'],'date':i['date'],'source':i['source'],'items':len(i['items']),'reconciled':i['reconciled'],'total':i['total']} for i in invoices]}
